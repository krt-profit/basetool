...
Every hangar mutation in this table — by the owner, by an admin on another member's hangar, the import and the fitted reset — is audited under `AuditDomain.HANGAR`; detaching a deleted ship from a mission unit also writes `MISSION_UNIT_UPDATED` (see [`docs/specs/audit.md`](docs/specs/audit.md)).
...
| Read **audit logs** (Bank/Lager/Aufträge/Raffinerie/Mein Inventar/Missionen/Operationen/Rollen/Beförderung/Materialbörse/Hangar) + time-range PDF/JSON + retention cleanup (`/admin/audit-log`, `/api/v1/audit/**`, URL **and** method gate `hasRole('ADMIN')`) |   ❌    |  ❌   | ❌  |    ❌    |   ✅   |
