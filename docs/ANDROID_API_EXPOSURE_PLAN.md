...
|----|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| A1 | ✅ **Done, PR #1566** — client-IP attribution: right-to-left `X-Forwarded-For` walk in a new `ClientIpContextFilter`, plus `forward-headers-strategy: none` and a re-registration of `ForwardedHeaderFilter` one slot behind it. **The original scoping of this row was wrong about the mechanism** — patching `RateLimitingFilter` alone would have changed nothing, because `ForwardedHeaderFilter` is pinned to `Integer.MIN_VALUE`, already overwrites `getRemoteAddr()` with the leftmost hop and hides the header, so the limiter's whole trusted-proxy branch is dead code behind a proxy (measured, pinned by `ForwardedHeaderRewriteTest`). Narrowing `app.rate-limit.trusted-proxies` from `172.28.0.0/16` is **deliberately deferred to D3**: the API vhost's proxy network does not exist yet, and narrowing early would cut the frontend's own relay | Without it a client-chosen leftmost entry mints a fresh bucket per request — rate limiting becomes decorative and foreign IPs get framed in the logs |
...
| A3 | ✅ **Done, PR #1571** — per-`sub` rate limiting behind authentication (service-level, ingest `SubjectRateLimiter` as the model) for the app-consumed write families and the SSE connect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | CGNAT puts many members behind one IPv4; a per-IP bucket either throttles innocents or is set so loose it stops mattering                            |
| A4 | ✅ **Done, PR #1569** — `Cache-Control: private, no-store` on the sensitive GET families (bank, member PII, notifications) — `ApiCacheControlFilter` emits only `no-cache, must-revalidate` today                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Storage-with-revalidation is allowed by the current header; a public ingress makes intermediaries plausible                                          |
...
| A6 | ✅ **Done, PR #1570** — reviewed the `PaginationUtil` clamp (`MAX_PAGE_SIZE` 100 000) for public ingress and capped the anonymous-reachable list endpoints                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | One request returning 100 k rows is an amplification lever that the query timeout bounds only in time                                                |
...
| A8 | ✅ **Done** — `basetool_api_client_requests_total{client_id}` (bounded `azp`, `ApiUnknownClient` on the `other` series) and `basetool_auth_failures_total{reason}` (RFC 6750 code behind each 401); the rate-limit rejection counter already existed and gained its `bucket=subject` series with A3. Shipped **ahead of the app** on purpose: the counter needs a baseline of today's traffic to be readable later (REQ-OBS-018). Original scope: per-`azp` client counter with an unknown-client alert, auth-failure counter, rate-limit rejection counter (ingest's `basetool_ingest_*` are the model)                                                                                                                                                                                                                                                          | The "detect" half of the abuse ladder; without it revocation has nothing to act on                                                                   |

A1–A4 and A6 are improvements on their own merits and landed before any exposure decision was
final — all five are on `main`. A8 followed them for a reason that only holds *before* the app
ships: a client-attribution counter is worth nothing without a baseline of the traffic that exists
today. A5 (min-version gate) needs the app's `User-Agent` convention and A7 (assetlinks) its
signing-certificate fingerprint, so those two genuinely wait for the app.

### Track B — requirements and decisions (code)

| #  |                                                                                                                                                                                                    Work package                                                                                                                                                                                                     |
|----|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| B1 | ✅ **Done — ADR-0135.** Expose `/api/v1` through a dedicated public vhost (with the rejected alternative — an ingest-style gateway — and why it buys no authorisation)                                                                                                                                                                                                                                               |
...
| B3 | ✅ **Done — ADR-0136 + REQ-API-009**, enumerated and guarded by `ExternalContractTest`. Original scope: ADR + REQ-API amendment: the **external contract set** — which endpoints the app consumes become a shipped contract that may only change through `/api/v2` + `@ApiDeprecation`. Today `docs/specs/api-conventions.md` lets frontend-only endpoints change shape in place, which a shipped app cannot survive |
...
| B5 | ✅ **Done — REQ-OBS-018.** Probes, metrics, alerts and log pipeline for the new surface. Original scope: probes, metrics, alerts and log pipeline for the new surface                                                                                                                                                                                                                                                |
...
✅ **Done, staged.** Everything below is written, reviewed and deployed; the probes for the vhost
itself ship **commented out** because `api.profit-base.online` does not resolve until D3/D4, and an
un-staged probe of a non-existent host pages from the minute it merges (REQ-OBS-014). The enable
procedure is [`MONITORING_ROLLOUT_RUNBOOK.md`, Appendix C](MONITORING_ROLLOUT_RUNBOOK.md) — it is
the first thing to run after D3 and D4, and it is all-or-nothing: a partially enabled probe set
reads as "monitored". The A8 counters and their alert ship live. Original scope:
...
Step-by-step operator procedure for D3/D4/D5 — including the phase order, the one step that
causes a brief full-stack outage, the exact NPM snippets and the rollback for each phase:
...
| D2 | ✅ **Done, 2026-08-17 — applied to the production `iri` realm** with `scripts/provision-keycloak-mobile-client.py` after E1 passed and a full rehearsal against a throwaway Keycloak 26.7 stood in for the test realm. Both client-policy lists were empty beforehand and were captured as the rollback basis; the independent `--verify-only` pass is clean. The client `basetool-android` now exists with the marker role, the `aud=basetool-backend` mapper and `offline_access` withheld — unused until the vhost of D3 goes live. **Re-run required (2026-08-21):** that run left the client's realm-role scope empty, which with `fullScopeAllowed: false` means its tokens carry no realm roles at all and every member signing in through the app is reconciled onto the `Guest` fallback in the database (REQ-SEC-035). The script now provisions and verifies the scope; the production realm needs one more `--profile prod` pass by @greluc before the app ships. Nothing is broken in production meanwhile — the client is still unused | Includes the `aud=basetool-backend` mapper, which must exist before D5                                                                        |
| D3 | **[owner]** NPM: new vhost `api.profit-base.online` → `https://backend:11261`, backend joins a dual-stack proxy network; the vhost **overwrites** the whole `X-Forwarded-*`/`Forwarded` family, denies `/actuator`, carries a stricter version-controlled budget for the Keycloak token endpoint and per-location `client_max_body_size` — ✅ **Done, 2026-08-21.** Confirmed from outside the network by the nightly `edge-deny-probe` run at 05:49Z: `api.profit-base.online/actuator/prometheus` and `/actuator/health` both answer **404**, so the host resolves, serves TLS and its `/actuator` deny holds.                                                                                                                                                                                                                                                                                                                                                                                                                                     | I deliver the snippet contents; the NPM admin database is host state                                                                          |
| D4 | **[owner]** DNS: A/AAAA records for the new host, plus a **CAA** record for `profit-base.online` (Let's Encrypt only) — ✅ **Done, 2026-08-21**, implied by the same external probe reaching the host at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Cheap, unrelated to the app, worth doing anyway                                                                                               |
| D5 | **[owner]** Flip `IRI_BACKEND_EXPECTED_AUDIENCES=basetool-backend` — **release gate**, not a later hardening step — ✅ **Done, 2026-08-21.** The release gate is closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | The vhost must not go live against a backend that accepts audience-less tokens from arbitrary realm clients                                   |

> **Anonymous surface (2026-08-21).** Two operations on the vhost answer without a token: `GET /api/v1/terms/document` (ADR-0138) and `GET /api/v1/missions/search`, whose guest-redacted rows the public home page already renders. Both are deliberate and enumerated in **REQ-SEC-037**; the rollout runbook's verification step reads its expected status from that table rather than assuming `401` for every path, which is what produced a false alarm the first time it was run.| D6 | **[owner]** Add the app to the approved client software list in the Terms of Use (REQ-SEC-027)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Policy lever against unofficial clients                                                                                                       |
...
| E2 | ✅ **Done** — tests for the A1 XFF walk (spoofed chains, trusted and untrusted peers) and for the A3 per-subject limiter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
