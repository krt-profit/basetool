...
|----|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| A1 | **[code]** Client-IP attribution: right-to-left `X-Forwarded-For` walk in a new `ClientIpContextFilter`, plus `forward-headers-strategy: none` and a re-registration of `ForwardedHeaderFilter` one slot behind it. **The original scoping of this row was wrong about the mechanism** — patching `RateLimitingFilter` alone would have changed nothing, because `ForwardedHeaderFilter` is pinned to `Integer.MIN_VALUE`, already overwrites `getRemoteAddr()` with the leftmost hop and hides the header, so the limiter's whole trusted-proxy branch is dead code behind a proxy (measured, pinned by `ForwardedHeaderRewriteTest`). Narrowing `app.rate-limit.trusted-proxies` from `172.28.0.0/16` is **deliberately deferred to D3**: the API vhost's proxy network does not exist yet, and narrowing early would cut the frontend's own relay | Without it a client-chosen leftmost entry mints a fresh bucket per request — rate limiting becomes decorative and foreign IPs get framed in the logs |
