...
- Ein profit-eligible SK; ein **SK-verantworteter** Auftrag (öffentlich). Die Aufträge legt `test-admin` per REST an.
...
1. **SK-öffentliche Warteschlange:** Officer (Staffel B) öffnet `/orders?status=OPEN`.
...
- **Shared-DB-robust:** Die Assertions prüfen je Auftrag die eigene `data-id`, sodass von Geschwistertests akkumulierte Aufträge die Erwartung nicht verfälschen.
