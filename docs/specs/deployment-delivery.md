...
6. **Record what would trigger a re-review** — the growth that would invalidate the multiple.
   ...
   with traffic. Tempo's 2026-08-29 re-measurement (#1705) is the worked example — three
   `grafana/tempo:3.0.2` generations peaked at 479.8 / 486.0 / 523.1 MiB and both `3.0.3`
   generations at 595.3 / 665.3 MiB, a ~27 % step at the version boundary that the previous reading
   had attributed to trace volume. `max_over_time(container_memory_rss{name="…"}[30d])` returns one
