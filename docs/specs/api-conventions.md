...
| `GET /api/v1/missions/lookup`                                 | `id`, `name`, `status` — **403** anonymously: `GET /missions/**` is `permitAll`, so the method guard refuses at the seam                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `GET /api/v1/operations/lookup`                               | `id`, `name` — the Operation picker on the Einsatz's Kern section                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `GET /api/v1/job-types`                                       | envelope `content`; row `id`, `name`, `active` — **anonymous** (REQ-SEC-037), a role-name catalogue like `/ship-types`. `active` is read as a FILTER, so losing it returns every retired Funktion to both pickers with nothing failing                                                                                                                                                                                                                                                                                                        |
...
| `POST /api/v1/missions/{id}/participants`                     | as `core` — **request** requires `userId`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `GET /api/v1/missions/{id}/unit-ship-options`                 | `id`, `name`, `shipType` — **403** anonymously: `GET /missions/**` is `permitAll`, so the method guard refuses at the seam. A ship without an id is dropped; the type is what tells two Carracks apart                                                                                                                                                                                                                                                                                                                                        |
| `POST` · `PUT` · `DELETE …/missions/{id}/units/…/slim`        | *(answers discarded; the Einsatz is re-read)* — **request** requires `name` on the create and the edit, `participantId` on the crew add, nothing on the rest. The full-DTO twins are deprecated with a sunset of 2026-10-20 and are **not** frozen                                                                                                                                                                                                                                                                                            |
