...
fires only while the gate is closed, the readable-documents exemption, fail-open, cache bound), `TermsAcceptanceQueryDataTest` + `TermsAcceptanceServiceTest` (append-only history,
