...
counters carry a bounded `topic_class` label (one of the ten `LiveSyncTopicClass` labels: `mission`,
`operation`, `order_detail`, `orders_queue`, `refinery_order`, `bank_account`, `bank_staff`,
`orgunit_bank`, `materialboard`, `inventory_all`), and
