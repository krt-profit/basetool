...
| 3 | **Recoverability** | The organisation's records exist in one place. Losing them is not survivable by re-entering them. | [`backup-recovery.md`](../specs/backup-recovery.md) `REQ-OPS-010`, the nightly restic backup and the restore drill that proves it |
| 4 | **Operability by one person** | There is one maintainer. Anything that needs a second pair of hands at 03:40 is a design fault. | Runbooks under `docs/`, the conformance gate, alerting that names the artefact rather than the symptom |
| 5 | **Auditability** | Who changed what, in the areas where that question is asked after the fact. | `REQ-AUDIT-001`, [`audit.md`](../specs/audit.md) — an append-only trail across nine audited areas |
