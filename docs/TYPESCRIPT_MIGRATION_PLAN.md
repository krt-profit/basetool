...
Three properties of the codebase, all measured rather than assumed (recounted 2026-09-22):
...
`tsc --noEmit` in the build gate (`:frontend:typecheckJs`, TypeScript 7), opt-in `// @ts-check`,
hand-written declarations for the cross-file and bootstrap contracts (`frontend/types/*.d.ts`), and
DTO types generated from `openapi.json` at build time by the project's own emitter
(`frontend/scripts/gen-api-types.mjs`, ADR-0130).
...
- Known work per file, measured on the files converted so far: DOM narrowing casts
  ...
  `{@link …}`, `@param name {shape}`) must be converted to real JSDoc in the 3 unchecked files that still use them.
...
Finish ADR-0069's extraction for the remaining ~2 480 lines across 60 templates (about 1 480 of
them already `th:inline="javascript"` bootstrap blocks, about 1 000 still plain script), using the
...
- Emit **classic scripts, not modules**, at this stage: same filenames, same load order, same
  end-of-body position. Nothing about the 114 `th:src` tags changes yet.
...
- **The DTO types keep coming from `openapi.json`.** Generation is orthogonal to the source
  language and stays exactly as ADR-0125 and ADR-0130 built it, in every phase.
...
| 2     | ~2 480 inline lines, 60 templates | Large                                         | Medium — behaviour-preserving extraction    |
| 3     | Strictness ratchet                | Medium                                        | Low                                         |
| 4     | Build step                        | Medium                                        | **High** — first change to asset production |
| 5     | 95 files `.js` → `.ts`            | Large but mechanical                          | Low once Phase 4 holds                      |
