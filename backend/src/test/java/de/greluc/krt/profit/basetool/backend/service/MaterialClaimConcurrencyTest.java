...
/**
 * Reproduces the concurrent <em>first-claim</em> race on the squadron sign-up path against the real
 * Postgres test container and pins its last-writer-wins guarantee: two logisticians of the same
 * squadron lodging the first {@link MaterialClaim} on one material bucket at the same instant must
 * <b>both</b> complete without any thread seeing a 500 / propagated conflict, and the DB must end
 * up with exactly one row for the {@code (bucket, squadron)} pair.
 *
 * <p>The row carries a JPA {@code @Version} (via {@code AbstractEntity}) <em>and</em> the unique
 * index {@code uq_material_claim_bucket_org_unit} (V131), so the loser of the parallel INSERT hits
 * a {@code DataIntegrityViolationException} (or, on the repeat edit, an {@code
 ...
 * transaction, so the loser reloads the winner's committed row and UPDATEs it in place. This test
 * is the dynamic regression guard for that behaviour; the deterministic retry-count contract lives
 * in {@code MaterialClaimServiceTest.UpsertClaimConcurrencyTests}.
 *
...
class MaterialClaimConcurrencyTest {
   ...
   * @param squadronId the single claiming squadron both writers act for
   * @param skId the responsible Spezialkommando
   */
  private record Fixture(UUID orderId, UUID materialId, UUID squadronId, UUID skId) {}
  ...
  private Fixture seed() {
    return transactionTemplate.execute(
        status -> {
          ...
          return new Fixture(order.getId(), mat.getId(), sq.getId(), sk.getId());
        });
  }

  /**
   * Builds a claim payload for the seeded bucket and squadron at the given amount.
   *
