...
/**
 * Exposes the calling user's own approval status, so the frontend can route a {@code PENDING} or
 * {@code REJECTED} user to the waiting-for-approval page. Open to any authenticated user, including
 * one holding only {@code ROLE_PENDING_APPROVAL}.
 */
@RestController
