 ...
 *
 * <p>The backend is not internet-reachable and every request that gets this far already carries a
 * valid Keycloak token, so the realistic actor here is an <em>authenticated squadron member</em>
 * typing into a search box, a filter field or a free-text form input — not an anonymous attacker.
 * That is precisely why the guard is needed rather than optional: such text is echoed into log
 * lines (a rejected search term, a validation failure, a conflict diagnostic), and a member could
 * otherwise paste a newline followed by a fabricated {@code ERROR ---} prefix and have it read as a
 * genuine second log line while someone triages an incident (CWE-117). Neither the logback pattern
 * nor the maskers strip a {@code \n}.
 *
 * <p>Lives in the dependency-free {@code support} leaf, not next to the maskers in {@code logging}:
 * {@code logging} depends on {@code service} (the correlation filter resolves the acting user and
 * org unit), so an {@code integration} or {@code service} class calling into {@code logging} would
 * close an {@code integration -> logging -> service -> integration} package cycle. ArchUnit
 * enforces that (ADR-0047), and this class has no dependencies of its own, so the leaf is its
 * natural home.
 *
 * <p>Complements the two maskers instead of replacing either. {@code PiiMasker} removes <em>secrets
 * and PII</em> from a line that already reached the appender; {@code LogMasker} redacts a
