...
// `terms-version.properties` is generated by the root `generateTermsVersion` task from the
// frontend's German message bundle and read at runtime by TermsVersionProvider (REQ-SEC-028).
// The backend is the only consumer on purpose — it is the single authority on whether a user has
// accepted, so the frontend and the ingest gateway learn the answer from it rather than deriving a
// second one. `withSourcesJar()` above makes `sourcesJar` a second consumer of the same directory,
// hence the dependency on both tasks rather than on `processResources` alone.
sourceSets.main { resources.srcDir(rootProject.layout.buildDirectory.dir("generated/terms")) }

tasks.named("processResources") { dependsOn(rootProject.tasks.named("generateTermsVersion")) }

tasks.named("sourcesJar") { dependsOn(rootProject.tasks.named("generateTermsVersion")) }
