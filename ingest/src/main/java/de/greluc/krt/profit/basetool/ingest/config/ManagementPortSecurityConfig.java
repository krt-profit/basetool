...
/**
 * Opens {@code /actuator/**} without authentication on the dedicated management port (ADR-0090),
 * active only when {@code management.server.port} is set. That port is reachable only from the
 * monitoring scrape network and localhost.
 */
@Configuration
...
public class ManagementPortSecurityConfig {
   ...
   * MonitoringScrapeSecurityConfig} and {@link SecurityConfig}. CSRF protection stays enabled; the
   * chain serves only GETs (REQ-OBS-016).
   *
