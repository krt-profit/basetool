 ...
 * Keycloak's unauthenticated internal management port 9000 (REQ-SEC-014). Without this chain Boot's
 * default security would 401 the {@code /actuator/prometheus} scrape on the management port (health
 * is open by default), so Prometheus — which sends no credentials on these jobs — could not scrape.
 */
@Configuration
