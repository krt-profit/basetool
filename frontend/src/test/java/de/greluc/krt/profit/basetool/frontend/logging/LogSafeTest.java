...
/**
 * Unit tests for the frontend log-injection guard. The text reaching these log lines is typed by an
 * authenticated member (or a guest holding an edit link) into a search box or form field, so the
 * threat is not an anonymous attacker but a user who can make the frontend log write a fabricated
 * {@code ERROR} line — which would be taken at face value while triaging an incident.
 */
class LogSafeTest {
  ...
  void leavesAnOrdinarySearchTermUntouched() {
    ...
    // otherwise a sanitised term no longer matches what the user actually typed.
