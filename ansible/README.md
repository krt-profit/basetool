...
| `25-scripts.yml` | `scripts` | `deploy.sh`, `backup.sh`, `restore-drill.sh`, `container-cleanup.sh`, `lib/container-runtime.sh`, `lib/common.sh`, `render-env-d.py`, `render-redis-acl.py` (root-owned, `0755`) and the Redis ACL rules template `redis-users.acl.tmpl` (`0644`); the four `iri-*` units and timers with their shared sandbox drop-in `10-deploy-account-sandbox.conf` (`basetool_host_deploy_account_units`), logrotate, the lock tmpfiles; timers **enabled, not started** |
