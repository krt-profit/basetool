...
| **DeployHealthRestartFailing** | The deployed release could not be restored: a container runs the target image but is unhealthy and the targeted restart will not stick, or (since 2026-09-25) a drift re-apply of the deployed release — a missing container, a wrong image — did not bring it up. A runtime fault, deliberately not a rollback: the rollback anchors still name the release before it. `{app="ops-deploy"} \|~ "health drift\|re-appl\|did not come up"` names the service(s). |
