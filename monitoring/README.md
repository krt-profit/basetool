...
| **AuditDomainSilenceAnomaly** | One audit domain recorded nothing for 30d while others stay active — it may have lost its audit wiring (REQ-AUDIT-001). Check the domain's `auditService.record` calls. `ROLE`/`PROMOTION`/`PERSONAL_INVENTORY`/`MARKET` are excluded; review their volume on the `07` per-domain tables. Same `keep_firing_for: 15m` hold. |
