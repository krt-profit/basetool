...
| **BankAuditSilenceAnomaly**                                                                                              | The bank audit trail (separate `bank_audit_event` table) recorded nothing for 5d while the backend is up — the most sensitive area's audit logging may be broken. Verify bank mutations still write `bank_audit_event` rows.                                                                                                 |
...
| **JobOrderStale / RefineryOrderStale / OperationStale**                                                                  | A work-queue item has been open/planned > 30d — likely abandoned. Close, reassign or cancel it. The 30d floor is a baseline; tune it to the org's cadence.                                                                                                                                                                   |
...
| **FrontendLoginBroken**                                                                                                  | The frontend logged login failures with zero successes in 15m — OAuth2 login is likely broken for everyone (code-to-token, JWKS or state failure). Check frontend logs, Keycloak reachability and the OIDC client config.                                                                                                    |
