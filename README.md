...
EDGE_HOST_FRONTEND                           # the five vhost names the edge serves — no defaults,
EDGE_HOST_KEYCLOAK                           # the edge refuses to start without them
EDGE_HOST_INGEST
EDGE_HOST_GRAFANA
EDGE_HOST_API
...
| `IRI_KEYCLOAK_HOST_ALIAS`                                                                            | `name:ip` entry so the apps resolve the public issuer name to the local proxy where NAT does not hairpin. Since ADR-0166 that is the WEB host — identity has no hostname of its own.                                                                                                                                                                                                                                                                                                                                                                                       | `localhost:127.0.0.1` (no-op)                |
...
| `IRI_EXTRA_JAVA_OPTS`                                                                                | Appended to `JAVA_TOOL_OPTIONS`; used to select a non-default trust store where the issuer is self-signed. Because it is appended last, it also overrides a base flag — e.g. `-XX:-UseCompactObjectHeaders` disables the object layout of ADR-0180, at the cost of the baked AppCDS archive.                                                                                                                                                                                                                                                                               | *(empty)*                                    |
