...
- **`docs` / `config` / `docker` / `design`** — specs, ADRs & arc42, static-analysis config, the edge (nginx) and ACME configuration, the maintenance page, the `basetool-config` and `keycloak-spi` bundle Dockerfiles and the committed test TLS material, and brand font sources.
