...
- **Lager (warehouse)** — org-scoped stock tracking on append-only entries (`/inventory`, `/inventory/my`, `/inventory/all`): book stock in/out, transfer it between users/locations/org units (Umbuchen), earmark quantity slices to job orders and missions (Variante C, REQ-INV-027) and merge stacks on write (REQ-INV-026). A stock row references either a **material** (with quality) or — since REQ-INV-029/ADR-0101 — a **game item** (blueprint output, whole units, no quality); item rows allocate only to ITEM job orders that request the item, a production booking (Herstellung) can book the produced items straight into the Lager for a chosen owner and location (REQ-INV-032), and the order detail page surfaces the stock earmarked to an ITEM order in an Item-Bestand panel with a per-entry delivered toggle (REQ-ORDERS-028), and delivering an item order consumes that earmarked stock best-effort — drawn down as it is handed over, oldest-first, without ever blocking a legacy line manufactured before item stock existed (REQ-ORDERS-030).
...
- **In-app notifications** — a data-driven notification system: an admin-managed rule engine (`/admin/notification-rules`) raises per-user notifications (e.g. bank booking requests, new Discord registrations, Materialbörse interest for the offer owner) into a personal inbox (`/notifications`) delivered via polling plus a live SSE push, with automatic 90-day cleanup (epic #622).
- **Live multi-user sync** — every surface where several people see the same state updates **in place for all viewers when a peer changes it, no manual reload**: the mission detail (join, crew, finance, schedule/status/party-lead/frequency/Ablauf/owning-org-unit), the operation detail (core data, payout mark), the job-order **queue** (new order — including anonymous guest creates — and drag-reorder, each viewer in their own filter/page) and **detail** (status, KPI band, assignees, materials, handovers, production, claims), the Kartellbank (bookings, confirmations, reversals, grants, management, requests, org-unit bank) and the Materialbörse board. It runs over one multiplexed `/ws/sync` WebSocket per tab, fanned across replicas by Redis pub/sub; when the viewer has a dialog open the refresh is deferred behind an „Aktualisierungen verfügbar" pill instead of yanking the DOM. Only opaque section keys cross the socket — every viewer re-pulls its own authorization-checked fragment (REQ-FE-015, [ADR-0093](docs/adr/0093-tool-wide-topic-room-live-sync-relay.md); epic #1102, closes #1115 / #1120).
- **Activity audit logs** — an immutable, append-only activity trail across ten areas (Bank, Lager, Aufträge, Raffinerie, Mein Inventar, Missionen, Operationen, Rollen & Mitglieder, Beförderung, Materialbörse), read on one ADMIN-only page (`/admin/audit-log`) with a per-area tab switcher and event-type/period/actor filters. Each log exports as PDF or JSON for a chosen period and supports an explicit, itself-logged retention purge of entries older than a cutoff (REQ-AUDIT-001/004, ADR-0037/0038, epics #795/#800).
...
- **Live multi-user sync rollout (one-time, network change):**
  [`docs/LIVESYNC_ROLLOUT_RUNBOOK.md`](docs/LIVESYNC_ROLLOUT_RUNBOOK.md). The #1102 live-sync + Redis
  fan-out ships with the app stack, but the backend joining the new `net-redis-backend` network needs
  a one-time manual clean `down` + `up` (an in-place `up` would strand it off Redis, #974); the
  runbook covers the pre-checks, the deploy and the rollback (REQ-FE-015, ADR-0093).
...
* **`scripts`** — Server-side operations layer: the GHCR-pull production deploy script (`deploy.sh`, invoked by the `iri-deploy` systemd timer), the weekly `docker-cleanup.sh` maintenance script (armed by the `iri-docker-cleanup` systemd timer), the `check-flyway-migrations.sh` guard, and their systemd / logrotate units.
...
| `KEYCLOAK_JWK_SET_URI`                       | (Backend + ingest, optional) When set, the JWT resource servers (backend and ingest) fetch the signing keys from this URL instead of deriving them from the issuer. Point it at the internal Keycloak (`https://keycloak:18443/realms/iri/protocol/openid-connect/certs`) so token validation does not hairpin through the public edge; requires the `keycloak-trust` bundle (cert SAN `dns:keycloak`). Empty derives the JWKS from `KEYCLOAK_ISSUER_URI` (REQ-SEC-024, ADR-0073). | *(unset -> issuer-derived JWKS)*                                |
...
| `MONITORING_SCRAPE_USER`                     | (Optional) Basic-auth username for the Prometheus scrape endpoint `/actuator/prometheus` (all three modules; REQ-OBS-005, ADR-0072). Fail-closed: while unset, the endpoint denies every request.                                                                                                                                                                                                                                                                                  | *(unset → endpoint denies all)*                                 |
...
| `MONITORING_TRACING_ENABLED`                 | (Optional) Master gate for distributed tracing (all three modules; REQ-OBS-009, ADR-0072). `false` keeps tracing fully inert — no tracer, no `traceparent` propagation, no exporter. Set `true` together with `MONITORING_OTLP_ENDPOINT` where the monitoring stack runs (epic #936 Phase 2).                                                                                                                                                                                      | `false`                                                         |
