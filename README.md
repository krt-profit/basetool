...
- **Live multi-user sync** — every surface where several people see the same state updates **in place for all viewers when a peer changes it, no manual reload**: the mission detail (join, crew, finance, schedule/status/party-lead/frequency/Ablauf/owning-org-unit), the operation detail (core data, payout mark), the job-order **queue** (new order — including anonymous guest creates — and drag-reorder, each viewer in their own filter/page) and **detail** (status, assignees, materials, handovers, claims), the Kartellbank (bookings, confirmations, reversals, grants, management, requests, org-unit bank) and the Materialbörse board. It runs over one multiplexed `/ws/sync` WebSocket per tab, fanned across replicas by Redis pub/sub; when the viewer has a dialog open the refresh is deferred behind an „Aktualisierungen verfügbar" pill instead of yanking the DOM. Only opaque section keys cross the socket — every viewer re-pulls its own authorization-checked fragment (REQ-FE-015, [ADR-0093](docs/adr/0093-tool-wide-topic-room-live-sync-relay.md); epic #1102, closes #1115 / #1120).
