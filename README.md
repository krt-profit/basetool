...
- **Materialbörse** — a central, org-wide-visible material-exchange trade board (`/materialboerse`, Flotte & Logistik). A member releases one of their own Lager rows for trade via the *Für Börse freigeben* checkbox (with a Markdown remark, max. 20 000 chars), choosing how much to offer — the whole row or only a **part** of its stock (the offered amount is editable later and capped at the row's current stock); the board shows only who offers which material in which quality and quantity, and other members register interest so the anbieter can open the negotiation. The anbieter is notified in-app when someone registers interest in one of their offers (REQ-MARKET-011). Handover and location stay off-tool and **private** — the item's Standort is never exposed — and interessenten names are visible only to the offer's owner. Master-detail UI with *Alle Angebote* / *Meine Angebote* tabs, search/quality/amount filters and server-rendered Markdown; every mutation is audited under `AuditDomain.MARKET` (REQ-MARKET-001…011, ADR-0082/0086).
