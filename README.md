...
| `IRI_BACKEND_KEYCLOAK_JWK_SET_URI` | Internal JWKS URL for the backend (`KEYCLOAK_JWK_SET_URI`, REQ-SEC-024): fetch the token-signing keys from `https://keycloak:18443/auth/realms/iri/protocol/openid-connect/certs` over the pinned `keycloak-trust` bundle instead of through the edge. Empty = issuer-location discovery, as before. Runbook: [`deployment.md` → *Internal JWKS for the backend*](docs/deployment.md#internal-jwks-for-the-backend). | empty |
...
| `IRI_TRUSTSTORE_HOST_PATH`                                                                           | Host path mounted at `/run/secrets/truststore.p12`. Build it from the JVM anchors plus your cert.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | same file as the keystore (the Quadlet units: the CA-only `/var/iri/secrets/tls/truststore.p12`, REQ-SEC-070) |
...
| `INTERNAL_TLS_VERIFY_HOSTNAME` | `true` makes the frontend's backend client, its readiness probe and the ingest relay verify the backend's hostname on top of the pinned anchor (REQ-SEC-070). Host `.env`, rendered into `env.d`; `dev`/`test` ignore it. | `true` in compose and the units since the per-service certificates (the apps' own default is `false`) |
