...
# NOT fix it either: docker-compose.monitoring.yml mounts prometheus.yml (and the alloy/blackbox
# configs) as SINGLE FILES, and `deploy.sh` writes the new config via mirror_dir/rsync, which
