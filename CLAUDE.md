...
Profit Basetool — a squadron-management web app (mission planning, hangar, inventory, refinery, user admin) for the "DAS KARTELL" / IRIDIUM organization. Three Spring Boot 4 applications (`backend`, `frontend`, `ingest`) on Java 25, plus the `keycloak-spi` provider JAR, the `keycloak-theme`, the shipped `logging-support` library and the test-only `test-support` library; PostgreSQL 18, Keycloak 26 OAuth2, Redis-backed Spring Sessions. Production runs as rootless Podman + Quadlet units generated from the compose files (arc42 §7); local and test stacks run on Docker Compose. Gradle 9 with Kotlin DSL. Dependency versions live in the **version catalog** at **`gradle/libs.versions.toml`** — edit that, not `build.gradle.kts`. [refreshVersions](https://jmfayard.github.io/refreshVersions/) runs in *catalog* mode and is applied **only on request**: `./gradlew refreshVersions -PrefreshVersions` annotates the catalog in place with `## ⬆ = "…"` comment markers for each available update and changes no version itself (without the flag the task does not exist — the plugin is kept off every other build because it is not configuration-cache compatible). **There is no `versions.properties`** (removed 2026-09-23): it held zero versions, and the refreshVersions run recreates an empty one that is gitignored. Repositories are declared once, in `settings.gradle.kts` (`FAIL_ON_PROJECT_REPOS`); shared module conventions (toolchain, Lombok/JetBrains, Mockito agent, `spotbugsMain`, SBOM settings) live in the root `build.gradle.kts` `subprojects { plugins.withId(...) }` blocks, not in the module scripts. CI builds with `--configuration-cache` (strict), so a build-script change must stay configuration-cache compatible — check with `./gradlew help --configuration-cache` run twice (the second must say *reused*) and `./gradlew build -m --configuration-cache`. **Every artifact Gradle resolves is checked against `gradle/verification-metadata.xml`** (SHA-256, strict; ADR-0208, REQ-OPS-034): a change that alters what the build resolves — a catalog bump, a new dependency or plugin — regenerates it in the same commit with `GRADLE_USER_HOME="$(mktemp -d)" ./gradlew --write-verification-metadata sha256 help build :frontend:compileE2eJava` (details: CONTRIBUTING → *Dependency verification*).
...
- **`logging-support`** — the one `LogSafe` + PII-masking layout/encoder; a plain library the three apps ship (ADR-0205). Log hygiene only — never domain code.
...
- **Never use production / real credentials in tests or local test stacks.** This is a hard rule. It applies to every kind of test (Mockito unit tests, MockMvc, `@SpringBootTest`, TestContainers integration tests) and to every manually-started local stack used to verify a change. Forbidden inputs include — non-exhaustively — the production `.env` at the repo root, the shared `keystore.p12` at `backend/src/main/resources/keystore.p12`, the shared `realm-export.json` Keycloak dump, real OIDC client secrets, real database passwords, real SMTP credentials, real Keycloak admin passwords, real JWT signing keys. Use dedicated test artifacts instead: `.env.test` (gitignored via `.env.*`), the **committed** TLS material in [`docker/test-tls/`](docker/test-tls/README.md), a stripped `realm-export.json` with rotated client secrets and a synthetic test user, the `docker-compose.test.yml` override. **The committed TLS material is not an exception to this rule but an application of it** ([ADR-0139](docs/adr/0139-shared-committed-tls-material-for-the-test-stack.md)): the rule forbids bringing a *real* credential into a test stack, and this is a throwaway built to be published — the CA key that signed it was destroyed at generation time, the server key can only serve loopback and docker-network names, and it is bound by a hardcoded path so `IRI_KEYSTORE_HOST_PATH` (which still selects the production keystore) cannot be pointed at a test stack by accident. Deliberately publishing a worthless artefact and leaking a real one are opposite acts; do not read one as licence for the other. The README's `Running the local test stack` section carries the full `.env.test` variable set and the realm-rewrite command; the TLS material needs no command at all. Reason: anything that enters a worktree, a CI log, a container volume, a screenshot artifact, an MCP-preview snapshot or an editor backup has to be assumed leaked and rotated — and the recovery is cheaper if it never had to happen in the first place. When you spin a local stack up to verify a UI change, always source `.env.test` (never `.env`), point Docker Compose at `--env-file .env.test`, and tear the stack down with `down --volumes` after the verification.
...
- **A Javadoc sits directly above the declaration it documents, in every source set.** `main` gets the full `google_checks.xml`; `test` and `e2e` run `config/checkstyle/javadoc_position.xml`, which holds only `InvalidJavadocPosition` ([ADR-0222](docs/adr/0222-every-source-set-fails-on-a-misplaced-javadoc.md)). An orphaned block, meaning a Javadoc directly above another Javadoc, fails `check`. It happens when code is inserted between a Javadoc and its member: move the block back onto the member it describes, delete it if that member is gone, and never merge the two.
